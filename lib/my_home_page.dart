  ...
  class _MyHomePageState extends State<MyHomePage> with SingleTickerProviderStateMixin {
    int _selectedIndex = 0;//cách Flutter biết người dùng đang bấm vào tab nào, từ đó hiển thị đúng trang tương ứng.
    ...
    //   });
    // }
    Map<String, List<Map<String, dynamic>>> groupSongsByCategory(List<Map<String, dynamic>> songs) {
  ...
  //Chuyển sang phát bài hát kế tiếp trong danh sách nhạc
    ...
    void playNext() {
      ...
      _currentIndex = (_currentIndex + 1) % playlist.length;// phát hết bài trong danh sách quay lại bài đầu tiên
      _player.seek(Duration.zero, index: _currentIndex + 1);//chuyển đến bài hát mới ở giây thứ 0
      //seek() là để tua
        ...
        onTap: () {
          ...
          setState(() {
            ...
            if (index == 2){
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => Music_Chart(
                  playlist: [],
    ...
    ReadData() async {
      ...
      String popularJson = await DefaultAssetBundle.of(context).loadString("json/popularBooks.json");
      String booksJson = await DefaultAssetBundle.of(context).loadString("json/books.json");
    ...
    Future<void> loadAllSongs() async {
      ...
      setState(() {
        chillSongs = allSongs.where((s) =>
        (s['category']?.toString().toLowerCase().trim() ?? '') == 'nhạc chill').toList();
        lightSongs = allSongs.where((s) =>
        (s['category']?.toString().toLowerCase().trim() ?? '') == 'nhạc nhẹ').toList();
        hotSongs = allSongs.where((s) =>
        (s['category']?.toString().toLowerCase().trim() ?? '') == 'nhạc hot').toList();
        loveSongs = allSongs.where((s) =>
        (s['category']?.toString().toLowerCase().trim() ?? '') == 'tình yêu').toList();
        studentSongs = allSongs.where((s) =>
        (s['category']?.toString().toLowerCase().trim() ?? '') == 'học trò').toList();
        lovelifeSongs = allSongs.where((s) =>
        (s['category']?.toString().toLowerCase().trim() ?? '') == 'yêu đời').toList();
      });
    }
                          ...
                          return [
                            SliverAppBar(
                              ...
                              bottom: PreferredSize(
                                preferredSize: Size.fromHeight(165), // Chiều cao lớn hơn để chứa ảnh
                                child: FutureBuilder(
                                  future: Supabase.instance.client.from('songs').select().limit(40),// giới hạn theo số bài
                                  builder: (context, snapshot) {
                                    if (!snapshot.hasData) return Center(child: CircularProgressIndicator());
                                    final data = snapshot.data as List;
                                    final categories = <String, Map<String, dynamic>>{};
                                    for (var song in data) {
                                      final cat = (song['category'] ?? '').toString().toLowerCase().trim();
                                      if (!categories.containsKey(cat)) {
                                        categories[cat] = song; // chỉ lấy bài đầu tiên của mỗi loại
                                      }
                                    }
                                    final uniqueSongs = categories.values.toList();
                                    ...
                                    for (var song in data) {
                                      final rawCat = (song['category'] ?? '').toString().toLowerCase().trim();
                                      if (normalized.containsKey(rawCat) && !categoryMap.containsKey(rawCat)) {
                                        categoryMap[rawCat] = song;
                                      }
                                    }
                                    //bọc 3 phần
                                    return SizedBox(
                                      height: 220, // Tăng một chút nếu muốn ảnh rõ hơn
                                      child: ListView.builder(
                                        ...
                                        itemBuilder: (context, index) {
                                          ...
                                          final rawCat = (song['category'] ?? '').toString().toLowerCase().trim();
                                          final displayCat = normalized[rawCat] ?? '';
                                          final String hexColor = song['color'] ?? '#D3D3D3';
                                            ...
                                            onTap: () {
                                              setState(() {
                                                ...
                                                _songsFuture = Supabase.instance.client
                                                    ...
                                                    .then((value) => List<Map<String, dynamic>>.from(value));// chỉ khi chắc chắn trả về Future<List>
                                              });
                                            },
                                            child: Container(
                                              width: 150,
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  Container(
                                                    ...
                                                    decoration: BoxDecoration(
                                                      color: Colors.white.withOpacity(0.2), // nền cố định
                                                      borderRadius: BorderRadius.circular(8),
                                                    ),
                                                    child: Text(
                                                      "CÓ THỂ BẠN THÍCH",
                            ...
                            if (selectedCategory == null)
                              ...
                              FutureBuilder<List<Map<String, dynamic>>>(
                                ...
                                builder: (context, snapshot) {
                                  ...
                                  if (data.isEmpty) {
                                    return Center(child: Text("Không có bài hát nào trong thể loại này."));
                                  }
    ...
    Widget buildSongItem(Map song, List<Map<String, dynamic>> playlist, int index) {
      return InkWell(
        onTap: () async {
          Navigator.push(
            context,
